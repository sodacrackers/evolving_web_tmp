...
export default class InsertThreeColGridCommand extends Command {
  execute() {
    const { model } = this.editor;

    model.change((writer) => {
      // Insert <threeColGrid>*</threeColGrid> at the current selection
...
function createGrid(writer) {
  ...
  // threecolgridediting.js.
  const grid = writer.createElement('threeColGrid');
  const leftGrid = writer.createElement('leftThreeColGrid');
  const centerGrid = writer.createElement('centerThreeColGrid');
  const rightGrid = writer.createElement('rightThreeColGrid');
  ...
  writer.appendElement('paragraph', centerGrid);
