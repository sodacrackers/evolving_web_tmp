...
export default class InsertTwoColLeftGridCommand extends Command {
  execute() {
    const { model } = this.editor;

    model.change((writer) => {
      // Insert <twoColLeftGrid>*</twoColLeftGrid> at the current selection
...
function createGrid(writer) {
  ...
  // twocolleftgridediting.js.
  const grid = writer.createElement('twoColLeftGrid');
  const leftGrid = writer.createElement('twoColLeftGridLeft');
  const rightGrid = writer.createElement('twoColLeftGridRight');
